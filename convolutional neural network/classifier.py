...
class CatClassifier(keras.Model):
    def __init__(self, num_classes, input_shape):
        ...
        self.conv_layers = keras.Sequential([
            layers.Conv2D(8, kernel_size=3, strides=1, padding='same', activation='relu', input_shape=input_shape),
            ...
            layers.MaxPooling2D(pool_size=2, strides=2),
            layers.BatchNormalization(),
            layers.Conv2D(64, kernel_size=5, strides=1, padding='same', activation='relu'),
            layers.MaxPooling2D(pool_size=2, strides=2),
